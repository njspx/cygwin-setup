...
void PickView::setViewMode (views mode)
{
  view_mode = mode;
  packagedb db;
...
const char * PickView::mode_caption (views mode)
{
  switch (mode) {
...
void PickView::setObsolete (bool doit)
{
  showObsolete = doit;
  refresh ();
}

void PickView::insert_pkg (packagemeta & pkg, int indent)
{
...
void PickView::insert_category (CategoryTree *cat_tree)
{
  if (!cat_tree)
    return;
  ...
  if ((!showObsolete && isObsolete(cat->first))) return;
  ...
  if (!isAll) {
    ...
    for (std::vector<packagemeta *>::const_iterator i = cat->second.begin();
         i != cat->second.end(); ++i) {
      if (packageFilterString.empty() ||
          (*i && StrStrI((*i)->name.c_str(), packageFilterString.c_str()))) {
        packageCount++;
      }
    }
  ...
  if (!isAll && !hasContents) return;

  // insert line for the category
  contents.push_back(
      new PickCategoryLine(*this, cat_tree, packageCount, isAll ? 0 : 1));
...
void PickView::init_headers (void)
{
  ...
  for (packagedb::packagecollection::iterator n = db.packages.begin();
    ...
    if (pkg.categories.size() > 2) {
      ...
      for (cnt = 0, cat = pkg.categories.begin();
           cnt < NUM_CATEGORY_COL_WIDTH && cat != pkg.categories.end(); ++cat) {
        if (casecompare(*cat, "All") == 0) continue;
        if (compound_cat.size()) compound_cat += ", ";
        compound_cat += *cat;
        cnt++;
      }
...
void PickView::init(views _mode, ListView *_listview, Window *_parent)
{
  view_mode = _mode;
  listview = _listview;
  parent = _parent;
...
void PickView::build_category_tree()
{
  ...
  for (packagedb::categoriesType::iterator n = packagedb::categories.begin();
       n != packagedb::categories.end(); ++n) {
    if (casecompare(n->first, "All") == 0) continue;
...
void PickView::defaultTrust (trusts trust)
{
  this->deftrust = trust;
}

void PickView::refresh()
{
