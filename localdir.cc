...
static void check_if_enable_next (HWND h)
{
...
static int offer_to_create (HWND h, const char *dirname)
{
  ...
  if (rv) note(h, IDS_CANT_MKDIR, dirname);

  return rv;
}

static int CALLBACK browse_cb (HWND h, UINT msg, LPARAM lp, LPARAM data)
{
  ...
  switch (msg) {
    ...
    case BFFM_VALIDATEFAILED:
      ...
      snprintf(tempname, sizeof tempname, "%s\\%s", dirname,
               reinterpret_cast<LPTSTR>(lp));
      if (!offer_to_create(h, tempname)) {
        SendMessage(h, BFFM_SETSELECTION, TRUE,
                    reinterpret_cast<LPARAM>(tempname));
        return -1;
      }
      // Reset to original directory instead.
      SendMessage(h, BFFM_SETSELECTION, TRUE,
                  reinterpret_cast<LPARAM>(dirname));
      return -1;
  }
  return 0;
}

static void browse (HWND h)
{
  BROWSEINFO bi;
  /* SHGetPathFromIDList doesn't handle path length > MAX_PATH. */
  CHAR name[MAX_PATH];
  ...
  bi.lpszTitle = (g_source != IDC_SOURCE_LOCALDIR)
                     ? "Select download directory"
                     : "Select local package directory";
  bi.ulFlags =
      BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE |
      ((g_source != IDC_SOURCE_LOCALDIR) ? (BIF_EDITBOX | BIF_VALIDATE) : 0);
  bi.lpfn = browse_cb;
  pidl = SHBrowseForFolder(&bi);
  if (pidl) {
    if (SHGetPathFromIDList(pidl, name)) eset(h, IDC_LOCAL_DIR, name);
  }
}
...
long LocalDirPage::OnNext() 
{
  HWND h = GetHWND();

  save_dialog(h);
  while (local_dir[local_dir.size() - 1] == '\\' ||
         local_dir[local_dir.size() - 1] == '/')
    ...
    if (attr != INVALID_FILE_ATTRIBUTES && (attr & FILE_ATTRIBUTE_DIRECTORY)) {
      ...
      if (g_source == IDC_SOURCE_LOCALDIR) {
        if (!unattended_mode) {
          ...
          char msgText[1000];
          LoadString(hinstance, IDS_NO_LOCALDIR, msgText, sizeof(msgText));
      ...
      if (FormatMessage(
              ...
              err, LANG_NEUTRAL, (LPSTR)&buf, 0, 0) != 0) {
        snprintf(msg, sizeof(msg), msgText, local_dir.c_str(), buf, err);
        LocalFree(buf);
      } else
        snprintf(msg, sizeof(msg), msgText, local_dir.c_str(),
                 "(unknown error)", err);
      Log(LOG_PLAIN) << msg << endLog;
      int ret = MessageBox(h, msg, 0, MB_ICONEXCLAMATION | MB_ABORTRETRYIGNORE);
