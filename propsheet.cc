  ...
  WPARAM wParam, LPARAM lParam)
{
  ...
  switch (uMsg) {
    case WM_SYSCOMMAND:
      if ((wParam & 0xfff0) == SC_CLOSE) goto areyousure;
      break;
    case WM_COMMAND:
      if (wParam != 2) break;
areyousure:
      if (MessageBox(hwnd,
                     ...
                     "download or installation will be aborted.",
    ...
    case WM_SIZE: {
      ...
      if (wParam != SIZE_MINIMIZED) {
        ...
        if (psd.clientRectValid) {
          const int dX = clientRect.width() - psd.lastClientRect.width();
          const int dY = clientRect.height() - psd.lastClientRect.height();

          ControlAdjuster::AdjustControls(hwnd, PropSheetControlsInfo, dX, dY);
          ...
          /*
            ...
            dialogs among them.
           */
          if (psd.gotPage) EnumChildWindows(hwnd, &EnumPages, 0);
        } else {
...
static int CALLBACK PropSheetProc (HWND hwndDlg, UINT uMsg, LPARAM lParam)
{
  switch (uMsg) {
    ...
    case PSCB_INITIALIZED: {
      ...
      */
      SendMessage(
          hwndDlg, WM_SETICON, ICON_BIG,
          (LPARAM)LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(IDI_CYGWIN)));
      /*
...
bool PropSheet::Create (const Window * Parent, DWORD Style)
{
  ...
  p.dwFlags = PSH_NOAPPLYNOW | PSH_WIZARD |
              PSH_USECALLBACK
              /*| PSH_MODELESS */
              | PSH_USEICONID;
  if (Parent != NULL) {
...
void PropSheet::SetHWNDFromPage (HWND h)
{
 ...
 */
void PropSheet::AdjustPageSize (HWND page)
{
  PropSheetData& psd = PropSheetData::Instance();
...
void PropSheet::AddPage (PropertyPage * p)
{
...
void PropSheet::SetButtons (DWORD flags)
{
  // Posts a message to the message queue, so this won't block
  PropSheet_SetWizButtons (GetHWND (), flags);
}

void PropSheet::PressButton (int button)
{
